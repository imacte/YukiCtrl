...
fn load_config() -> (HotplugToggles, HotplugThresholds) {
    let path = config_path();
    let content = match fs::read_to_string(&path) {
        Ok(s) => s,
        Err(e) => {
            debug!("hotplug config not loaded ({}), using defaults", e);
            return (HotplugToggles::default(), HotplugThresholds::default());
        }
    };
        ...
        if let Some((k, v)) = line.split_once(':') {
            ...
            match k {
                ...
                "off_threshold_idle_pct" => {
                    if let Ok(x) = v.parse::<f32>() { thresholds.off_threshold_idle_pct = x; }
                }
                "on_threshold_util_pct" => {
                    if let Ok(x) = v.parse::<f32>() { thresholds.on_threshold_util_pct = x; }
                }
                _ => {}
            }
        }
...
fn write_state_yaml(
    ...
    let path = state_path();
    let body = format!(
        "# Auto-generated by yumi hotplug loop. Do not edit.\n\
         online_mask: {:#x}\n\
         thermal_c: {:.1}\n\
         lockscreen_onoff: {}\n\
         screens_onoff: {}\n\
...
fn run_one_tick(state: &mut HotplugLoopState) -> Result<()> {
    let (toggles, mut thresholds) = load_config();
    ...
    let fas_panic_now = fas::is_fas_panic();
    let enabled = toggles.lockscreen_onoff && toggles.screens_onoff;

    // 漏洞 1: 读全局触摸信号
    let touch_down = touch_signal::is_touch_down();
    ...
    if state.last_state_write.map_or(true, |t| now.duration_since(t) >= Duration::from_millis(500)) {
        ...
        if let Err(e) = write_state_yaml(mask, thermal_c, toggles, thresholds) {
...
pub fn init_state_file() -> Result<()> {
    let _ = fs::create_dir_all(hotplug_dir());
    let body = "# yumi hotplug state (will be populated on first daemon tick)\n\
                 online_mask: 0x0\n\
                 thermal_c: 0.0\n";
    let p = state_path();
