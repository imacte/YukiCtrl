...
# 本地可选配置文件目录
LOCAL_CONFIGS_DIR="$MODPATH/configs"
# 临时下载位置
TEMP_DOWNLOAD_FILE="/data/local/tmp/yumi_config_download.yaml"
...
# 新增：本地配置检查提示
MSG_PREPARE_LOCAL="-> 3. Checking for local config..."
MSG_LOCAL_FOUND="   - ✔ Local config found:"
MSG_LOCAL_VALID_FAIL="   - ❌ Local config validation failed. Falling back to download..."
MSG_LOCAL_CLEAN="-> Cleaning up local configs directory..."

MSG_PREPARE_DOWNLOAD="-> 4. Preparing to download:"
...
MSG_DOWNLOAD_FAIL="-> ❌ Failed to find or download specific config."
...
if echo "$CURRENT_LOCALE" | $BUSYBOX grep -qi "zh"; then
  ...
  # 新增：本地配置检查提示
  MSG_PREPARE_LOCAL="-> 3. 正在检查本地配置..."
  MSG_LOCAL_FOUND="   - ✔ 找到本地配置:"
  MSG_LOCAL_VALID_FAIL="   - ❌ 本地配置校验失败。将尝试从网络下载..."
  MSG_LOCAL_CLEAN="-> 正在清理本地 configs 目录..."

  MSG_PREPARE_DOWNLOAD="-> 4. 准备下载:"
...
check_local_config() {
    local target_name="$1"
    local local_file="$LOCAL_CONFIGS_DIR/$target_name.yaml"

    ui_print " "
    ui_print "$MSG_PREPARE_LOCAL"

    if [ -f "$local_file" ]; then
        ui_print " $MSG_LOCAL_FOUND $target_name.yaml"
        # 借用现有的校验函数，确保本地文件格式也正确
        if validate_config_content "$local_file" "$target_name"; then
            $BUSYBOX mv -f "$local_file" "$FINAL_CONFIG_PATH"
            return 0
        else
            ui_print " $MSG_LOCAL_VALID_FAIL"
            return 1
        fi
    fi
...
if [ -z "$TARGET_CONFIG_NAME" ]; then
    ...
    # 步骤 3: 优先检查本地 configs 文件夹
    if check_local_config "$TARGET_CONFIG_NAME"; then
        CONFIG_APPLIED=true
        ui_print " "
        ui_print "$MSG_DOWNLOAD_APPLY_SUCCESS $TARGET_CONFIG_NAME (Local)"
        ui_print " $MSG_DOWNLOAD_APPLY_PATH $FINAL_CONFIG_PATH"
    else
        # 步骤 4: 如果本地没有或者校验失败，尝试下载
        if download_config "$TARGET_CONFIG_NAME"; then
            CONFIG_APPLIED=true
            ui_print " "
            ui_print "$MSG_DOWNLOAD_APPLY_SUCCESS $TARGET_CONFIG_NAME (Downloaded)"
            ui_print " $MSG_DOWNLOAD_APPLY_PATH $FINAL_CONFIG_PATH"
        fi
    fi
fi

# 步骤 5: 处理结果 (如果本地和下载均失败则回退)
...
# 步骤 6: 统一清理本地 configs 文件夹
if [ -d "$LOCAL_CONFIGS_DIR" ]; then
    ui_print " "
    ui_print "$MSG_LOCAL_CLEAN"
    $BUSYBOX rm -rf "$LOCAL_CONFIGS_DIR"
fi
